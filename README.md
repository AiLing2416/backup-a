...
- `latest`：默认指向最新版 `AMD64` 架构镜像。
- `AMD64`：最新版 `AMD64` 架构镜像。
- `ARM64`：最新版 `ARM64` 架构镜像。
- `1.4-AMD64`：`1.4` 版本的 `AMD64` 架构镜像。
- `1.4-ARM64`：`1.4` 版本的 `ARM64` 架构镜像。
- `1.4`：默认指向 `1.4` 版本的 `AMD64` 架构镜像。

当前发布同时推送到两个镜像仓库：

- `docker.io/ailing2416/backup-a`
- `ghcr.io/ailing2416/backup-a`

如果你的环境没有明确的架构需求，直接使用 `latest` 即可；如果你需要精确锁定版本或架构，请使用带版本号和架构后缀的标签。
